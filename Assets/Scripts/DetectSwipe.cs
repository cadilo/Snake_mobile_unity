    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        if((Mathf.Abs(eventData.delta.x)) > (Mathf.Abs(eventData.delta.y)))
        {
            if(eventData.delta.x > 0)
            {
                ...
                MultiPllayerController.direction = Vector2.right;
            }
            else
            {
                Debug.Log("left");
                Snake.direction = Vector2.left;
                MultiPllayerController.direction = Vector2.left;
            }
        }
        else
        {
            if (eventData.delta.y > 0)
            {
                ...
                MultiPllayerController.direction = Vector2.up;
            }
            else
            {
                Debug.Log("down");
                Snake.direction = Vector2.down;
                MultiPllayerController.direction = Vector2.down;
            }
        }
    }
